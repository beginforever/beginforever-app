// Begin Forever — Admin v14
// Fix: approved_at set to null on revoke/pending moves (not on approve)
// Fix: adAct() uses explicit field sets per action

async function ldAdmin(filter) {
  // Highlight active tab button
  ['adTabPending','adTabApproved','adTabRejected','adTabFounders'].forEach(function(id) {
    var el = document.getElementById(id);
    if (el) el.className = 'btn btn-sm ' + (id === 'adTab' + filter.charAt(0).toUpperCase() + filter.slice(1) ? 'btn-gold' : 'btn-dark');
  });
...
// ── adAct: explicit field sets per action — approved_at null on non-approve
async function adAct(id, status) {
  ...
  if (status === 'approved') {
    ...
    // Revoke, pending, deleted — clear approved_at
  ...
  try {
    ...
    // Notify user via edge function (approve/reject only)
...
async function submitReject() {
  ...
  try {
    var r = await sb.from('profiles').update({
      ...
      approved_at: null   // clear approved_at on rejection
...
// ── Account management (deactivate/delete modals — called from hamburger menu)
