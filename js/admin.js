...
async function ldAdmin(st){
  ...
  d.forEach(function(p){
    ...
    var founderBadge=p.is_founding_member?'<span style="font-size:9px;padding:2px 7px;border-radius:8px;background:rgba(212,160,23,.15);color:var(--gold2);font-weight:700;margin-left:5px;">✦ Founder #'+p.founding_number+'</span>':'';
    ...
    l.innerHTML+=
      ...
      '</div><span style="font-size:10px;padding:4px 10px;border-radius:20px;font-weight:700;background:'+sc+';color:'+tc+';">'+st.toUpperCase()+'</span>'+
      '</div>'+details+photoHtml+idHtml+act+'</div>';
  });
}
...
async function ldFounders(){
  ...
  l.innerHTML+=
    ...
    '</div>'+
    '<div style="margin-bottom:14px;"><div style="display:flex;justify-content:space-between;font-size:10px;color:var(--w40);margin-bottom:4px;"><span>Progress</span><span>'+appr+' / '+FOUNDER_CAP+'</span></div>'+
    '<div style="height:6px;background:rgba(255,255,255,.08);border-radius:3px;overflow:hidden;"><div style="height:100%;width:'+pct+'%;background:linear-gradient(90deg,#D4A017,#F5C842);border-radius:3px;"></div></div></div>';
  d.forEach(function(p){
    var f=faithByKey(p.religion||'Other');
    l.innerHTML+='<div class="card" style="margin-bottom:8px;border:1px solid rgba(212,160,23,.12);display:flex;gap:10px;align-items:center;">'+
      ...
      '<div style="flex:1;min-width:0;"><p style="font-size:13px;font-weight:700;color:#fff;margin:0;">'+p.full_name+', '+p.age+'</p>'+
      '<p style="font-size:11px;color:'+f.color+';margin:1px 0;">'+f.icon+' '+(p.religion||'')+(p.denomination?' · '+p.denomination:'')+'</p>'+
      '<p style="font-size:10px;color:var(--w40);margin:0;">'+p.city+', '+p.state+'</p></div>'+
      '<span style="font-size:9px;padding:3px 8px;border-radius:10px;font-weight:700;background:rgba(39,174,96,.15);color:#4ade80;white-space:nowrap;">APPROVED ✓</span></div>';
  });
}
...
function openRejectModal(id,name,email){
  ...
  var el=document.getElementById('rejectModalName');if(el)el.textContent=name;
