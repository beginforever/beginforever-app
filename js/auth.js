...
// ── After OTP verified — create Supabase account
...
async function doLogin() {
  var input = document.getElementById('lEmail').value.trim();
  var pw    = document.getElementById('lPass').value;
  var errEl = document.getElementById('lErr');
  errEl.style.display = 'none';

  if (!input || !pw) {
    errEl.textContent = 'Please enter your email or phone and password.';
  ...
  try {
    ...
    var isPhone = /^[\d\s\+\-]{7,15}$/.test(input);
    if (isPhone) {
      var normalized = input.trim().replace(/\s+/g,'');
      ...
      var lookup = await sb.from('profiles').select('email').eq('phone', normalized).limit(1);
      if (!lookup.data || !lookup.data.length) {
        lookup = await sb.from('profiles').select('email').eq('phone', '+91'+normalized).limit(1);
      }
      if (!lookup.data || !lookup.data.length) {
        errEl.textContent = 'No account found with this phone number.';
        errEl.style.display = 'block';
        btn.disabled = false; btn.textContent = origText; btn.style.opacity = '1';
        return;
      }
      em = lookup.data[0].email;
    }
