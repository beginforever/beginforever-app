...
async function loadP() {
  ...
  // ✦ BATCH 2: Mandatory onboarding flow if approved + not yet completed
...
// ═══════════════════════════════════════════ SETUP WIZARD (unchanged from your live)
...
async function goNext(){
  ...
  if (!U) {
    try {
      ...
      if(e){e.textContent='Session error.';e.style.display='block';} btn.disabled=false; btn.textContent='Submit for Review ✦'; return;
    }
  }
  try{
    var urls=['','','','',''];
...
function renderFaithPrefCard(){
  ...
  el.innerHTML=
    '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">'+
      ...
      '<p style="font-size:11px;color:var(--w40);margin:3px 0 0;">Who you see &amp; who can reach you</p></div>'+
      editBtn+
    '</div>'+
    '<div style="'+(premium?'':'opacity:.45;filter:grayscale(.4);')+'">'+
      '<div style="margin-bottom:10px;"><p style="font-size:9px;font-weight:700;color:var(--w40);text-transform:uppercase;letter-spacing:1px;margin-bottom:6px;">🔍 Browsing profiles from</p>'+
      '<div id="fpBrowsePills" style="display:flex;flex-wrap:wrap;gap:5px;"></div></div>'+
      '<div><p style="font-size:9px;font-weight:700;color:var(--w40);text-transform:uppercase;letter-spacing:1px;margin-bottom:6px;">💌 Receiving interest from</p>'+
      '<div id="fpReceivePills" style="display:flex;flex-wrap:wrap;gap:5px;"></div></div>'+
    '</div>';
...
// ═══════════════════════════════════════════ INLINE PROFILE EDIT MODE
// User taps "Edit Profile" → profile tab switches to editable form fields inline.
// Tap "Save" → persists. Tap "Cancel" → reverts to view mode.

var _editMode = false;
var _editHobbies = [];
var _editAgeRanges = [];
var _editMaritalStatuses = [];
...
async function saveEditInline() {
  if (_editHobbies.length < 5) { alert('Please select at least 5 hobbies.'); return; }
  if (_editHobbies.length > 12) { alert('Maximum 12 hobbies.'); return; }
  if (_editAgeRanges.length < 1) { alert('Please select at least one preferred age range.'); return; }
  if (_editMaritalStatuses.length < 1) { alert('Please select at least one preferred marital status.'); return; }

  var btn = document.getElementById('eSaveBtn');
  if (btn) { btn.disabled = true; btn.textContent = 'Saving…'; }

  try {
    var minH = parseInt((document.getElementById('e_prefHeightMin')||{}).value) || 152;
    var maxH = parseInt((document.getElementById('e_prefHeightMax')||{}).value) || 196;
    ...
    var upd = {
      ...
      phone: (document.getElementById('e_phone')||{}).value || P.phone,
      profile_for: (document.getElementById('e_profileFor')||{}).value || P.profile_for,
      height_cm: parseInt((document.getElementById('e_height')||{}).value) || P.height_cm,
      income_bracket: (document.getElementById('e_income')||{}).value || P.income_bracket,
      looking_for_intent: (document.getElementById('e_lookingForIntent')||{}).value || P.looking_for_intent,
      ...
      pref_age_ranges: JSON.stringify(_editAgeRanges),
      pref_height_min_cm: minH,
      pref_height_max_cm: maxH,
      pref_marital_statuses: JSON.stringify(_editMaritalStatuses),
...
function renP() {
  ...
  var rows = [
   ...
   {l:'Annual Income',v:P.income_bracket?incomeBracketLabel(P.income_bracket):''},
   {l:'Mother Tongue',v:P.mother_tongue},
   {l:'Marital Status',v:P.marital_status},
   {l:'Looking For',v:P.looking_for_intent?lookingForLabel(P.looking_for_intent):''}
  ];
  ...
  // Partner preferences card (NEW)
  var ppEl = document.getElementById('profilePartnerPrefs');
  if (ppEl) {
    var prAges = []; try { prAges = JSON.parse(P.pref_age_ranges || '[]'); } catch(e) {}
    var prMar = []; try { prMar = JSON.parse(P.pref_marital_statuses || '[]'); } catch(e) {}
    if (prAges.length || prMar.length || P.pref_height_min_cm) {
      ppEl.style.display = '';
      var html = '';
      if (prAges.length) html += '<p style="font-size:11px;color:var(--w70);margin:4px 0;"><strong style="color:#fff;">Age:</strong> ' + prAges.map(function(a){return a==='50_plus'?'50+':a;}).join(', ') + '</p>';
      if (P.pref_height_min_cm && P.pref_height_max_cm) html += '<p style="font-size:11px;color:var(--w70);margin:4px 0;"><strong style="color:#fff;">Height:</strong> ' + cmToFtIn(P.pref_height_min_cm) + ' – ' + cmToFtIn(P.pref_height_max_cm) + '</p>';
      if (prMar.length) html += '<p style="font-size:11px;color:var(--w70);margin:4px 0;"><strong style="color:#fff;">Marital:</strong> ' + prMar.join(', ') + '</p>';
      if (P.pref_city && P.pref_city !== 'Any') html += '<p style="font-size:11px;color:var(--w70);margin:4px 0;"><strong style="color:#fff;">City:</strong> ' + P.pref_city + '</p>';
      var pc = document.getElementById('partnerPrefsContent');
      if (pc) pc.innerHTML = html;
    } else ppEl.style.display = 'none';
  }

  // Faith & beliefs (Church Attendance + Baptised REMOVED per request)
  ...
  // Lifestyle (Exercise stays, others reorganized — exercise removed per simplification)
...
function renPEditMode() {
  var heightOpts = HEIGHT_OPTIONS.map(function(h) {
    return '<option value="' + h.cm + '"' + (P.height_cm && Math.abs(P.height_cm - h.cm) < 2 ? ' selected' : '') + '>' + h.label + '</option>';
  }).join('');
  var profForOpts = PROFILE_FOR_OPTIONS.map(function(o) {
    var cur = P.profile_for || P.registered_by || 'Myself';
    return '<option value="' + o + '"' + (cur === o ? ' selected' : '') + '>' + o + '</option>';
  }).join('');
  var incomeOpts = INCOME_BRACKETS.map(function(b) {
    return '<option value="' + b.v + '"' + (P.income_bracket === b.v ? ' selected' : '') + '>' + b.l + '</option>';
  }).join('');
  var lfOpts = LOOKING_FOR_OPTIONS.map(function(o) {
    return '<option value="' + o.v + '"' + (P.looking_for_intent === o.v ? ' selected' : '') + '>' + o.l + '</option>';
  }).join('');
  var prefMinCm = P.pref_height_min_cm || 152;
  var prefMaxCm = P.pref_height_max_cm || 196;
  var prefMinOpts = HEIGHT_OPTIONS.map(function(h) { return '<option value="' + h.cm + '"' + (prefMinCm === h.cm ? ' selected' : '') + '>' + h.label + '</option>'; }).join('');
  var prefMaxOpts = HEIGHT_OPTIONS.map(function(h) { return '<option value="' + h.cm + '"' + (prefMaxCm === h.cm ? ' selected' : '') + '>' + h.label + '</option>'; }).join('');
  ...
  var heroEl = document.getElementById('profileHero');
  if (heroEl) heroEl.innerHTML = '<p style="font-family:Cinzel,serif;font-size:18px;color:var(--gold-bright);">✏️ Edit Profile</p><p style="font-size:11px;color:var(--w50);margin-top:4px;">All fields below are editable — tap Save when done.</p>';
  ...
  mi.innerHTML =
    ...
    '<div class="field-group"><label class="field-label">This profile is for</label><select class="field" id="e_profileFor">'+profForOpts+'</select></div>' +
    '<div class="field-group"><label class="field-label">Height</label><select class="field" id="e_height">'+heightOpts+'</select></div>' +
    '<div class="field-group"><label class="field-label">Annual Income</label><select class="field" id="e_income"><option value="">Select</option>'+incomeOpts+'</select></div>' +
    ...
    '<div class="field-group"><label class="field-label">Phone</label><input class="field" id="e_phone" value="' + (P.phone||'') + '"/></div>' +
    ...
    '<div class="field-group"><label class="field-label">Favourite Scripture / Verse</label><input class="field" id="e_scripture" value="' + (P.scripture||'') + '"/></div>' +
  ...
  if (hobEl) {
    hobEl.style.display = '';
    hobEl.innerHTML =
      '<p style="font-size:10px;font-weight:700;color:var(--gold);text-transform:uppercase;letter-spacing:1px;margin:0 0 4px;">Hobbies (min 5, max 12)</p>' +
  ...
  if (lfEl) {
    lfEl.style.display = '';
    lfEl.innerHTML =
      '<p style="font-size:10px;font-weight:700;color:var(--gold);text-transform:uppercase;letter-spacing:1px;margin:0 0 8px;">What I\'m Looking For</p>' +
      '<div class="field-group"><label class="field-label">Intent</label><select class="field" id="e_lookingForIntent"><option value="">Select</option>' + lfOpts + '</select></div>' +
      '<div class="field-group"><label class="field-label">Describe in your own words</label><textarea class="field" id="e_lookingForText" style="min-height:80px;resize:vertical;">' + (P.looking_for||'') + '</textarea></div>';
  }
  ...
  if (ppEl) {
    ppEl.style.display = '';
    ppEl.innerHTML =
      '<p style="font-size:10px;font-weight:700;color:var(--gold);text-transform:uppercase;letter-spacing:1px;margin:0 0 6px;">Partner Preferences</p>' +
      '<p style="font-size:11px;color:var(--w40);margin-bottom:12px;font-style:italic;">These help us find profiles where love truly begins to last.</p>' +
      '<div class="field-group"><label class="field-label">Preferred age ranges</label><div id="eAgeChips" style="display:flex;flex-wrap:wrap;gap:4px;"></div></div>' +
      '<div class="field-group"><label class="field-label">Preferred height range</label>' +
        '<div class="field-row">' +
          '<div><p style="font-size:10px;color:var(--w50);margin-bottom:4px;">Min</p><select class="field" id="e_prefHeightMin">' + prefMinOpts + '</select></div>' +
          '<div><p style="font-size:10px;color:var(--w50);margin-bottom:4px;">Max</p><select class="field" id="e_prefHeightMax">' + prefMaxOpts + '</select></div>' +
        '</div></div>' +
      '<div class="field-group"><label class="field-label">Preferred marital status</label><div id="eMaritalChips" style="display:flex;flex-wrap:wrap;gap:4px;"></div></div>' +
      '<div class="field-group"><label class="field-label">Preferred City</label><input class="field" id="e_prefCity" value="' + (P.pref_city||'') + '" placeholder="Any"/></div>';
    renderEditAgeChips();
    renderEditMaritalChips();
  }

  // Replace bottom buttons with Save / Cancel
  var fb = document.getElementById('profileFaithSummary');
  if (fb) fb.style.display = 'none';
  var btnContainer = document.getElementById('profileButtonsBlock') || (function(){
    var d = document.createElement('div');
    d.id = 'profileButtonsBlock';
    mi.parentNode.appendChild(d);
    return d;
  })();
  btnContainer.innerHTML =
    '<button id="eSaveBtn" class="btn btn-gold" style="margin-top:14px;" onclick="saveEditInline()">Save Changes ✦</button>' +
    '<button class="btn btn-dark" style="margin-top:10px;" onclick="closeEditInline()">Cancel</button>';
}

function renderEditHobbyChips() {
  var c = document.getElementById('eHobbyChips'); if (!c) return;
  c.innerHTML = '';
...
function renderEditAgeChips() {
  var c = document.getElementById('eAgeChips'); if (!c) return;
  c.innerHTML = '';
  AGE_RANGES.forEach(function(r) {
    var on = _editAgeRanges.indexOf(r.v) > -1;
    var label = r.v === '50_plus' ? '50+' : r.v;
    var chip = document.createElement('button');
    chip.type = 'button'; chip.textContent = label;
    chip.style.cssText = 'padding:6px 11px;border-radius:20px;font-size:11px;font-weight:700;cursor:pointer;font-family:Nunito,sans-serif;margin:2px;border:1px solid '+(on?'var(--gold)':'rgba(255,255,255,.18)')+';background:'+(on?'rgba(212,160,23,.2)':'rgba(255,255,255,.05)')+';color:'+(on?'#F5C842':'rgba(255,255,255,.5)')+';';
    chip.onclick = function() {
      var ix = _editAgeRanges.indexOf(r.v);
      if (ix > -1) _editAgeRanges.splice(ix, 1); else _editAgeRanges.push(r.v);
      renderEditAgeChips();
    };
    c.appendChild(chip);
  });
...
function renderEditMaritalChips() {
  var c = document.getElementById('eMaritalChips'); if (!c) return;
  c.innerHTML = '';
  MARITAL_STATUSES.forEach(function(m) {
    var on = _editMaritalStatuses.indexOf(m) > -1;
    var chip = document.createElement('button');
    chip.type = 'button'; chip.textContent = m;
    chip.style.cssText = 'padding:6px 10px;border-radius:20px;font-size:10px;font-weight:700;cursor:pointer;font-family:Nunito,sans-serif;margin:2px;border:1px solid '+(on?'var(--gold)':'rgba(255,255,255,.18)')+';background:'+(on?'rgba(212,160,23,.2)':'rgba(255,255,255,.05)')+';color:'+(on?'#F5C842':'rgba(255,255,255,.5)')+';';
    chip.onclick = function() {
      var ix = _editMaritalStatuses.indexOf(m);
      if (ix > -1) _editMaritalStatuses.splice(ix, 1); else _editMaritalStatuses.push(m);
      renderEditMaritalChips();
    };
    c.appendChild(chip);
  });
...
// Legacy editPhotos picker — kept for the old modal (not used in inline mode but referenced elsewhere)
...
// Keep closeEdit for backward-compat (old modal close)
function closeEdit(){
...
// ═══════════════════════════════════════════ PROFILE VIEW MODAL (with new fields)
async function viewProfile(id){
  ...
  [{l:'Bio',v:p.bio},{l:'Education',v:p.education},{l:'Occupation',v:p.occupation},
   ...
   {l:'Height',v:p.height_cm?p.height_cm+' cm ('+cmToFtIn(p.height_cm)+')':''},
   {l:'Looking For',v:p.looking_for_intent?lookingForLabel(p.looking_for_intent):''}].forEach(function(dd){
...
function renderRejectedScreen(profile){
  ...
  el.innerHTML=
    ...
    '<div style="background:rgba(212,160,23,.08);border:1px solid rgba(212,160,23,.2);border-radius:14px;padding:16px;margin-bottom:24px;text-align:left;">'+
      ...
      '<p style="font-size:12px;color:rgba(255,255,255,.6);line-height:1.9;">✓ Fix the issue mentioned above<br/>✓ Re-upload a clear government ID<br/>✓ Ensure your photo clearly shows your face<br/>✓ Resubmit — reviewed within 24 hrs</p>'+
    '</div>'+
...
function renderHobbyChips(containerId, selectedArr) {
  ...
  ALL_HOBBIES.forEach(function(h) {
    ...
    var chip = document.createElement('button');
    chip.type = 'button'; chip.textContent = h;
    chip.style.cssText = 'padding:6px 12px;border-radius:20px;font-size:11px;font-weight:700;cursor:pointer;font-family:Nunito,sans-serif;transition:all .15s;margin:3px;border:1px solid '+(on?'var(--gold)':'rgba(255,255,255,.18)')+';background:'+(on?'rgba(212,160,23,.2)':'rgba(255,255,255,.05)')+';color:'+(on?'#F5C842':'rgba(255,255,255,.5)')+';';
    chip.onclick = function() {
