...
async function loadP() {
  ...
  if (P.status === 'rejected') { showScr('rejectedScreen'); return; }
  ...
  if (P.is_admin) {
    ...
    if (!document.getElementById('adTab')) {
...
function pickId(inp) {
  ...
  document.getElementById('idN').textContent = '📄 ' + f.name;
...
async function goNext() {
  ...
  if (step === 1) {
    if (!document.getElementById('fName').value.trim() || !document.getElementById('fAge').value ||
      ...
      e.textContent = 'Please fill all required fields.'; e.style.display = 'block'; return;
    }
    step++; updUI(); return;
  }
  if (step === 2) { step++; updUI(); return; }
  if (step === 3) { if (!photos[0]) { e.textContent = 'Primary photo is required.'; e.style.display = 'block'; return; } step++; updUI(); return; }
  if (step === 4) { if (!document.getElementById('fIdT').value || !idFile) { e.textContent = 'ID type and upload are required.'; e.style.display = 'block'; return; } step++; updUI(); return; }
  if (step === 5) { step++; updUI(); return; }
  if (setupBrowse.length === 0) { e.textContent = 'Please select at least one faith to browse.'; e.style.display = 'block'; return; }

  var btn = document.getElementById('nxBtn');
  btn.disabled = true; btn.innerHTML = '<div style="width:16px;height:16px;border:2px solid rgba(255,255,255,.2);border-top-color:var(--gold2);border-radius:50%;animation:spin .6s linear infinite;margin:0 auto;"></div>';
  ...
  try {
    ...
    try {
      await fetch(SB_URL+'/functions/v1/smart-function', {
        ...
        body:JSON.stringify({full_name:pd.full_name, email:pd.email, city:pd.city, state:pd.state, denomination:pd.denomination||pd.religion, founding_number:foundingNum})
      });
    } catch(x) {}
...
function renP() {
  ...
  var h = '';
  [{l:'Email',v:P.email},{l:'Phone',v:P.phone},{l:'Religion',v:P.religion},{l:'Denomination',v:P.denomination},{l:'Age',v:P.age},{l:'Education',v:P.education},{l:'Occupation',v:P.occupation},{l:'Mother Tongue',v:P.mother_tongue},{l:'Marital Status',v:P.marital_status},{l:'Height',v:P.height_cm?P.height_cm+' cm':''}].forEach(function(d){
    if (d.v) h += '<div style="display:flex;justify-content:space-between;align-items:center;padding:7px 0;border-bottom:1px solid var(--w05);"><span style="font-size:10px;color:var(--w50);text-transform:uppercase;letter-spacing:.5px;">'+d.l+'</span><span style="font-size:13px;color:var(--w80);font-weight:600;">'+d.v+'</span></div>';
  });
...
async function saveEdit() {
  ...
  P = r2.data[0]; closeEdit(); renP(); alert('Profile updated! ✅');
}
...
function renderFpCards(cid, arr) {
  var FP = [{k:'Christian',i:'✝️',c:'#9B59B6'},{k:'Hindu',i:'🕉️',c:'#E07020'},{k:'Muslim',i:'☪️',c:'#2E8B57'},{k:'Sikh',i:'☬',c:'#C8960C'},{k:'Jain',i:'🕊️',c:'#4A90D9'},{k:'Buddhist',i:'☸️',c:'#D4700A'},{k:'Parsi',i:'🔥',c:'#C0392B'},{k:'Jewish',i:'✡️',c:'#2980B9'},{k:'Bahai',i:'⭐',c:'#8E44AD'},{k:'Spiritual',i:'🌱',c:'#27AE60'},{k:'Other',i:'🌐',c:'#888'}];
...
async function viewProfile(id) {
  ...
  [{l:'Bio',v:p.bio},{l:'Education',v:p.education},{l:'Occupation',v:p.occupation},{l:'Mother Tongue',v:p.mother_tongue},{l:'Marital Status',v:p.marital_status},{l:'Height',v:p.height_cm?p.height_cm+' cm':''}].forEach(function(dd){
...
async function ldViews() {
  ...
  d.forEach(function(v){
    ...
    var f = faithByKey(p.religion||'Other');
    l.innerHTML += '<div class="card" style="cursor:pointer" onclick="viewProfile(\''+p.id+'\')"><div style="display:flex;gap:10px;align-items:center"><div class="avatar" style="'+(p.photo_url?'background-image:url('+p.photo_url+')':'')+';border-color:'+f.color+';">'+(p.photo_url?'':'<span style="font-size:18px;opacity:.3">👤</span>')+'</div><div style="flex:1"><h3 style="font-size:14px;margin:0;font-weight:600;color:#fff;">'+p.full_name+', '+p.age+'</h3><p style="font-size:11px;color:'+f.color+';">'+f.icon+' '+(p.denomination||p.religion||'')+'</p></div><p style="font-size:10px;color:var(--w50);">'+new Date(v.viewed_at).toLocaleDateString()+'</p></div></div>';
  });
}

// Stubs for locked features
